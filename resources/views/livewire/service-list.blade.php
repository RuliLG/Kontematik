<nav>
    @if ($layout === 'list')
    <input type="search" wire:model="query" class="block border-gray-300 rounded mb-8 w-full" placeholder="{{ __('app.search_tool') }}">
                ...
                @foreach ($categories as $category)
                    ...
                    <li class="p-4 w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/5">
                        ...
                        >
                            ...
                            <span class="font-bold text-sm">{{ (new Translation())->getOrTranslate($s->name) }}</span>
                        </a>
                    </li>
                    @endforeach
                @endforeach
